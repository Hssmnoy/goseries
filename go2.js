...
if(fs.existsSync(DATA_DIR+"/progress.json")){

try{

progress=JSON.parse(fs.readFileSync(DATA_DIR+"/progress.json"))
...
const jsonFile=DATA_DIR+"/goseries4k_"+group+".json"
...
const file=DATA_DIR+"/iptv_"+group+".m3u"
...
const serverMatches = [...epHtml.matchAll(/data-id="(https?:\/\/[^"]+)"/g)]
let serverList = [...new Set(serverMatches.map(m => m[1]))]
...
const line2=`#EXTINF:-1 tvg-name="${epName}" tvg-logo="${poster}" group-title="${title}",${epName} (ok-nah)\n${s}\n\n`
...
fs.writeFileSync(
DATA_DIR+"/goseries4k_"+group+".json",
...
const file=DATA_DIR+"/goseries4k_"+group+".json"
